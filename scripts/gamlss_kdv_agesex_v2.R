...
# read the dataset:
dat_kdv_raw<-read.csv("dat_kdv_brainvol.csv")
dat_kdv_raw$Age<-as.numeric(dat_kdv_raw$Age)
dat_kdv_raw$Sex<-as.factor(dat_kdv_raw$Sex)
dat_kdv_raw$group<-as.factor(dat_kdv_raw$group)
dat_kdv_raw$dataset<-as.factor(dat_kdv_raw$dataset)
dat_kdv_raw$subject<-as.factor(dat_kdv_raw$subject)

drops <- c("Age","Sex","group","dataset","subject","scanner")
dat_kdv_nodemographics<-dat_kdv_raw[ , !(names(dat_kdv_raw) %in% drops)]
dat_kdv_demographics<-dat_kdv_raw[drops]

# scanner harmonization (Combat):

## transpose data:

t_dat_kdv_nodemographics <- transpose(dat_kdv_nodemographics)

## get row and colnames in order
colnames(t_dat_kdv_nodemographics) <- rownames(dat_kdv_nodemographics)
rownames(t_dat_kdv_nodemographics) <- colnames(dat_kdv_nodemographics)

## harmonization
mod <- model.matrix(~dat_kdv_demographics$Age+dat_kdv_demographics$Sex+dat_kdv_demographics$group)
t_dat_kdv_harmonized_nodemographics <- neuroCombat(dat=t_dat_kdv_nodemographics, batch=dat_kdv_demographics$scanner, mod=mod)
t_dat_kdv_harmonized_nodemographics <-t_dat_kdv_harmonized_nodemographics$dat.combat
t_dat_kdv_harmonized_nodemographics<-as.data.frame(t_dat_kdv_harmonized_nodemographics)

## transpose data back:

dat_kdv_harmonized_nodemographics <- transpose(t_dat_kdv_harmonized_nodemographics)

## get row and colnames in order:
colnames(dat_kdv_harmonized_nodemographics) <- rownames(t_dat_kdv_harmonized_nodemographics)
rownames(dat_kdv_harmonized_nodemographics) <- colnames(t_dat_kdv_harmonized_nodemographics)

## harmonized dataset:
dat_kdv_harmonized<-cbind(dat_kdv_demographics,dat_kdv_harmonized_nodemographics)
dat_kdv_control<-dat_kdv_harmonized[dat_kdv_raw$group=='control',]
dat_kdv_patient<-dat_kdv_harmonized[dat_kdv_raw$group=='patient',]

## proportions:

dat_kdv_control$left.caudate.prop<-dat_kdv_control$left.caudate/dat_kdv_control$total.intracranial
dat_kdv_patient$left.caudate.prop<-dat_kdv_patient$left.caudate/dat_kdv_patient$total.intracranial

dat_kdv_control$right.caudate.prop<-dat_kdv_control$right.caudate/dat_kdv_control$total.intracranial
dat_kdv_patient$right.caudate.prop<-dat_kdv_patient$right.caudate/dat_kdv_patient$total.intracranial

dat_kdv_control$left.hippocampus.prop<-dat_kdv_control$left.hippocampus/dat_kdv_control$total.intracranial
dat_kdv_patient$left.hippocampus.prop<-dat_kdv_patient$left.hippocampus/dat_kdv_patient$total.intracranial

dat_kdv_control$right.hippocampus.prop<-dat_kdv_control$right.hippocampus/dat_kdv_control$total.intracranial
dat_kdv_patient$right.hippocampus.prop<-dat_kdv_patient$right.hippocampus/dat_kdv_patient$total.intracranial
...
## left caudate: mBCPE
op <- par(mfrow = c(4, 3))
mNO <- histDist(dat_kdv_control$left.caudate, "NO", density = TRUE, main = "(a)",ymax=ymax)
mGA <- histDist(dat_kdv_control$left.caudate, "GA", density = TRUE, main = "(b)",ymax=ymax)
mBCPE <- histDist(dat_kdv_control$left.caudate, "BCPE", density = TRUE, main = "(c)",ymax = ymax)
mBCCG <- histDist(dat_kdv_control$left.caudate, "BCCG", density = TRUE, main = "(d)",ymax = ymax)
mGG <- histDist(dat_kdv_control$left.caudate, "GG", density = TRUE, main = "(e)",ymax = ymax)
mGIG <- histDist(dat_kdv_control$left.caudate, "GIG", density = TRUE, main = "(f)",ymax = ymax)
mIG <- histDist(dat_kdv_control$left.caudate, "IG", density = TRUE, main = "(g)",ymax = ymax)
mLOGNO <- histDist(dat_kdv_control$left.caudate, "LOGNO", density = TRUE, main = "(h)",ymax = ymax)
mWEI <- histDist(dat_kdv_control$left.caudate, "WEI", density = TRUE, main = "(i)",ymax = ymax)
mBCCGo <- histDist(dat_kdv_control$left.caudate, "BCCGo", density = TRUE, main = "(j)",ymax = ymax)
par(op)
...
## right.caudate: mBCPE
op <- par(mfrow = c(4, 3))
mNO <- histDist(dat_kdv_control$right.caudate, "NO", density = TRUE, main = "(a)",ymax=ymax)
mGA <- histDist(dat_kdv_control$right.caudate, "GA", density = TRUE, main = "(b)",ymax=ymax)
mBCPE <- histDist(dat_kdv_control$right.caudate, "BCPE", density = TRUE, main = "(c)",ymax = ymax)
mBCCG <- histDist(dat_kdv_control$right.caudate, "BCCG", density = TRUE, main = "(d)",ymax = ymax)
mGG <- histDist(dat_kdv_control$right.caudate, "GG", density = TRUE, main = "(e)",ymax = ymax)
mGIG <- histDist(dat_kdv_control$right.caudate, "GIG", density = TRUE, main = "(f)",ymax = ymax)
mIG <- histDist(dat_kdv_control$right.caudate, "IG", density = TRUE, main = "(g)",ymax = ymax)
mLOGNO <- histDist(dat_kdv_control$right.caudate, "LOGNO", density = TRUE, main = "(h)",ymax = ymax)
mWEI <- histDist(dat_kdv_control$right.caudate, "WEI", density = TRUE, main = "(i)",ymax = ymax)
mBCCGo <- histDist(dat_kdv_control$right.caudate, "BCCGo", density = TRUE, main = "(j)",ymax = ymax)
par(op)

GAIC(mNO, mGA, mBCPE, mBCCG, mGG, mGIG, mIG, mLOGNO, mWEI,mBCCGo)


## left.hippocampus: mGA
op <- par(mfrow = c(4, 3))
mNO <- histDist(dat_kdv_control$left.hippocampus, "NO", density = TRUE, main = "(a)",ymax=ymax)
mGA <- histDist(dat_kdv_control$left.hippocampus, "GA", density = TRUE, main = "(b)",ymax=ymax)
mBCPE <- histDist(dat_kdv_control$left.hippocampus, "BCPE", density = TRUE, main = "(c)",ymax = ymax)
mBCCG <- histDist(dat_kdv_control$left.hippocampus, "BCCG", density = TRUE, main = "(d)",ymax = ymax)
mGG <- histDist(dat_kdv_control$left.hippocampus, "GG", density = TRUE, main = "(e)",ymax = ymax)
mGIG <- histDist(dat_kdv_control$left.hippocampus, "GIG", density = TRUE, main = "(f)",ymax = ymax)
mIG <- histDist(dat_kdv_control$left.hippocampus, "IG", density = TRUE, main = "(g)",ymax = ymax)
mLOGNO <- histDist(dat_kdv_control$left.hippocampus, "LOGNO", density = TRUE, main = "(h)",ymax = ymax)
mWEI <- histDist(dat_kdv_control$left.hippocampus, "WEI", density = TRUE, main = "(i)",ymax = ymax)
mBCCGo <- histDist(dat_kdv_control$left.hippocampus, "BCCGo", density = TRUE, main = "(j)",ymax = ymax)
par(op)
...
## right.hippocampus: mGA
op <- par(mfrow = c(4, 3))
mNO <- histDist(dat_kdv_control$right.hippocampus, "NO", density = TRUE, main = "(a)",ymax=ymax)
mGA <- histDist(dat_kdv_control$right.hippocampus, "GA", density = TRUE, main = "(b)",ymax=ymax)
mBCPE <- histDist(dat_kdv_control$right.hippocampus, "BCPE", density = TRUE, main = "(c)",ymax = ymax)
mBCCG <- histDist(dat_kdv_control$right.hippocampus, "BCCG", density = TRUE, main = "(d)",ymax = ymax)
mGG <- histDist(dat_kdv_control$right.hippocampus, "GG", density = TRUE, main = "(e)",ymax = ymax)
mGIG <- histDist(dat_kdv_control$right.hippocampus, "GIG", density = TRUE, main = "(f)",ymax = ymax)
mIG <- histDist(dat_kdv_control$right.hippocampus, "IG", density = TRUE, main = "(g)",ymax = ymax)
mLOGNO <- histDist(dat_kdv_control$right.hippocampus, "LOGNO", density = TRUE, main = "(h)",ymax = ymax)
mWEI <- histDist(dat_kdv_control$right.hippocampus, "WEI", density = TRUE, main = "(i)",ymax = ymax)
mBCCGo <- histDist(dat_kdv_control$right.hippocampus, "BCCGo", density = TRUE, main = "(j)",ymax = ymax)
par(op)

GAIC(mNO, mGA, mBCPE, mBCCG, mGG, mGIG, mIG, mLOGNO, mWEI,mBCCGo)


## ctx.lh.superiorparietal: mGA
op <- par(mfrow = c(4, 3))
mNO <- histDist(dat_kdv_control$ctx.lh.superiorparietal, "NO", density = TRUE, main = "(a)",ymax=ymax)
mGA <- histDist(dat_kdv_control$ctx.lh.superiorparietal, "GA", density = TRUE, main = "(b)",ymax=ymax)
mBCPE <- histDist(dat_kdv_control$ctx.lh.superiorparietal, "BCPE", density = TRUE, main = "(c)",ymax = ymax)
mBCCG <- histDist(dat_kdv_control$ctx.lh.superiorparietal, "BCCG", density = TRUE, main = "(d)",ymax = ymax)
mGG <- histDist(dat_kdv_control$ctx.lh.superiorparietal, "GG", density = TRUE, main = "(e)",ymax = ymax)
mGIG <- histDist(dat_kdv_control$ctx.lh.superiorparietal, "GIG", density = TRUE, main = "(f)",ymax = ymax)
mIG <- histDist(dat_kdv_control$ctx.lh.superiorparietal, "IG", density = TRUE, main = "(g)",ymax = ymax)
mLOGNO <- histDist(dat_kdv_control$ctx.lh.superiorparietal, "LOGNO", density = TRUE, main = "(h)",ymax = ymax)
mWEI <- histDist(dat_kdv_control$ctx.lh.superiorparietal, "WEI", density = TRUE, main = "(i)",ymax = ymax)
mBCCGo <- histDist(dat_kdv_control$ctx.lh.superiorparietal, "BCCGo", density = TRUE, main = "(j)",ymax = ymax)
par(op)

GAIC(mNO, mGA, mBCPE, mBCCG, mGG, mGIG, mIG, mLOGNO, mWEI,mBCCGo)


## ctx.rh.superiorparietal: mBCPE
op <- par(mfrow = c(4, 3))
mNO <- histDist(dat_kdv_control$ctx.rh.superiorparietal, "NO", density = TRUE, main = "(a)",ymax=ymax)
mGA <- histDist(dat_kdv_control$ctx.rh.superiorparietal, "GA", density = TRUE, main = "(b)",ymax=ymax)
mBCPE <- histDist(dat_kdv_control$ctx.rh.superiorparietal, "BCPE", density = TRUE, main = "(c)",ymax = ymax)
mBCCG <- histDist(dat_kdv_control$ctx.rh.superiorparietal, "BCCG", density = TRUE, main = "(d)",ymax = ymax)
mGG <- histDist(dat_kdv_control$ctx.rh.superiorparietal, "GG", density = TRUE, main = "(e)",ymax = ymax)
mGIG <- histDist(dat_kdv_control$ctx.rh.superiorparietal, "GIG", density = TRUE, main = "(f)",ymax = ymax)
mIG <- histDist(dat_kdv_control$ctx.rh.superiorparietal, "IG", density = TRUE, main = "(g)",ymax = ymax)
mLOGNO <- histDist(dat_kdv_control$ctx.rh.superiorparietal, "LOGNO", density = TRUE, main = "(h)",ymax = ymax)
mWEI <- histDist(dat_kdv_control$ctx.rh.superiorparietal, "WEI", density = TRUE, main = "(i)",ymax = ymax)
mBCCGo <- histDist(dat_kdv_control$ctx.rh.superiorparietal, "BCCGo", density = TRUE, main = "(j)",ymax = ymax)
par(op)
...
## ctx.lh.parsopercularis: mBCPE

op <- par(mfrow = c(4, 3))
mNO <- histDist(dat_kdv_control$ctx.lh.parsopercularis, "NO", density = TRUE, main = "(a)",ymax=ymax)
mGA <- histDist(dat_kdv_control$ctx.lh.parsopercularis, "GA", density = TRUE, main = "(b)",ymax=ymax)
mBCPE <- histDist(dat_kdv_control$ctx.lh.parsopercularis, "BCPE", density = TRUE, main = "(c)",ymax = ymax)
mBCCG <- histDist(dat_kdv_control$ctx.lh.parsopercularis, "BCCG", density = TRUE, main = "(d)",ymax = ymax)
mGG <- histDist(dat_kdv_control$ctx.lh.parsopercularis, "GG", density = TRUE, main = "(e)",ymax = ymax)
mGIG <- histDist(dat_kdv_control$ctx.lh.parsopercularis, "GIG", density = TRUE, main = "(f)",ymax = ymax)
mIG <- histDist(dat_kdv_control$ctx.lh.parsopercularis, "IG", density = TRUE, main = "(g)",ymax = ymax)
mLOGNO <- histDist(dat_kdv_control$ctx.lh.parsopercularis, "LOGNO", density = TRUE, main = "(h)",ymax = ymax)
mWEI <- histDist(dat_kdv_control$ctx.lh.parsopercularis, "WEI", density = TRUE, main = "(i)",ymax = ymax)
mBCCGo <- histDist(dat_kdv_control$ctx.lh.parsopercularis, "BCCGo", density = TRUE, main = "(j)",ymax = ymax)
par(op)

GAIC(mNO, mGA, mBCPE, mBCCG, mGG, mGIG, mIG, mLOGNO, mWEI,mBCCGo)


## ctx.rh.parsopercularis: mNO

op <- par(mfrow = c(4, 3))
mNO <- histDist(dat_kdv_control$ctx.rh.parsopercularis, "NO", density = TRUE, main = "(a)",ymax=ymax)
mGA <- histDist(dat_kdv_control$ctx.rh.parsopercularis, "GA", density = TRUE, main = "(b)",ymax=ymax)
mBCPE <- histDist(dat_kdv_control$ctx.rh.parsopercularis, "BCPE", density = TRUE, main = "(c)",ymax = ymax)
mBCCG <- histDist(dat_kdv_control$ctx.rh.parsopercularis, "BCCG", density = TRUE, main = "(d)",ymax = ymax)
mGG <- histDist(dat_kdv_control$ctx.rh.parsopercularis, "GG", density = TRUE, main = "(e)",ymax = ymax)
mGIG <- histDist(dat_kdv_control$ctx.rh.parsopercularis, "GIG", density = TRUE, main = "(f)",ymax = ymax)
mIG <- histDist(dat_kdv_control$ctx.rh.parsopercularis, "IG", density = TRUE, main = "(g)",ymax = ymax)
mLOGNO <- histDist(dat_kdv_control$ctx.rh.parsopercularis, "LOGNO", density = TRUE, main = "(h)",ymax = ymax)
mWEI <- histDist(dat_kdv_control$ctx.rh.parsopercularis, "WEI", density = TRUE, main = "(i)",ymax = ymax)
mBCCGo <- histDist(dat_kdv_control$ctx.rh.parsopercularis, "BCCGo", density = TRUE, main = "(j)",ymax = ymax)
par(op)

GAIC(mNO, mGA, mBCPE, mBCCG, mGG, mGIG, mIG, mLOGNO, mWEI,mBCCGo)


## ctx.lh.superiortemporal: mIG/mGA/mLOGNO

op <- par(mfrow = c(4, 3))
mNO <- histDist(dat_kdv_control$ctx.lh.superiortemporal, "NO", density = TRUE, main = "(a)",ymax=ymax)
mGA <- histDist(dat_kdv_control$ctx.lh.superiortemporal, "GA", density = TRUE, main = "(b)",ymax=ymax)
mBCPE <- histDist(dat_kdv_control$ctx.lh.superiortemporal, "BCPE", density = TRUE, main = "(c)",ymax = ymax)
mBCCG <- histDist(dat_kdv_control$ctx.lh.superiortemporal, "BCCG", density = TRUE, main = "(d)",ymax = ymax)
mGG <- histDist(dat_kdv_control$ctx.lh.superiortemporal, "GG", density = TRUE, main = "(e)",ymax = ymax)
mGIG <- histDist(dat_kdv_control$ctx.lh.superiortemporal, "GIG", density = TRUE, main = "(f)",ymax = ymax)
mIG <- histDist(dat_kdv_control$ctx.lh.superiortemporal, "IG", density = TRUE, main = "(g)",ymax = ymax)
mLOGNO <- histDist(dat_kdv_control$ctx.lh.superiortemporal, "LOGNO", density = TRUE, main = "(h)",ymax = ymax)
mWEI <- histDist(dat_kdv_control$ctx.lh.superiortemporal, "WEI", density = TRUE, main = "(i)",ymax = ymax)
mBCCGo <- histDist(dat_kdv_control$ctx.lh.superiortemporal, "BCCGo", density = TRUE, main = "(j)",ymax = ymax)
par(op)

GAIC(mNO, mGA, mBCPE, mBCCG, mGG, mGIG, mIG, mLOGNO, mWEI,mBCCGo)


## ctx.rh.superiortemporal: mGA/mGIG/mGG

op <- par(mfrow = c(4, 3))
mNO <- histDist(dat_kdv_control$ctx.rh.superiortemporal, "NO", density = TRUE, main = "(a)",ymax=ymax)
mGA <- histDist(dat_kdv_control$ctx.rh.superiortemporal, "GA", density = TRUE, main = "(b)",ymax=ymax)
mBCPE <- histDist(dat_kdv_control$ctx.rh.superiortemporal, "BCPE", density = TRUE, main = "(c)",ymax = ymax)
mBCCG <- histDist(dat_kdv_control$ctx.rh.superiortemporal, "BCCG", density = TRUE, main = "(d)",ymax = ymax)
mGG <- histDist(dat_kdv_control$ctx.rh.superiortemporal, "GG", density = TRUE, main = "(e)",ymax = ymax)
mGIG <- histDist(dat_kdv_control$ctx.rh.superiortemporal, "GIG", density = TRUE, main = "(f)",ymax = ymax)
mIG <- histDist(dat_kdv_control$ctx.rh.superiortemporal, "IG", density = TRUE, main = "(g)",ymax = ymax)
mLOGNO <- histDist(dat_kdv_control$ctx.rh.superiortemporal, "LOGNO", density = TRUE, main = "(h)",ymax = ymax)
mWEI <- histDist(dat_kdv_control$ctx.rh.superiortemporal, "WEI", density = TRUE, main = "(i)",ymax = ymax)
mBCCGo <- histDist(dat_kdv_control$ctx.rh.superiortemporal, "BCCGo", density = TRUE, main = "(j)",ymax = ymax)
par(op)
...
## ctx.lh.supramarginal: mGA

op <- par(mfrow = c(4, 3))
mNO <- histDist(dat_kdv_control$ctx.lh.supramarginal, "NO", density = TRUE, main = "(a)",ymax=ymax)
mGA <- histDist(dat_kdv_control$ctx.lh.supramarginal, "GA", density = TRUE, main = "(b)",ymax=ymax)
mBCPE <- histDist(dat_kdv_control$ctx.lh.supramarginal, "BCPE", density = TRUE, main = "(c)",ymax = ymax)
mBCCG <- histDist(dat_kdv_control$ctx.lh.supramarginal, "BCCG", density = TRUE, main = "(d)",ymax = ymax)
mGG <- histDist(dat_kdv_control$ctx.lh.supramarginal, "GG", density = TRUE, main = "(e)",ymax = ymax)
mGIG <- histDist(dat_kdv_control$ctx.lh.supramarginal, "GIG", density = TRUE, main = "(f)",ymax = ymax)
mIG <- histDist(dat_kdv_control$ctx.lh.supramarginal, "IG", density = TRUE, main = "(g)",ymax = ymax)
mLOGNO <- histDist(dat_kdv_control$ctx.lh.supramarginal, "LOGNO", density = TRUE, main = "(h)",ymax = ymax)
mWEI <- histDist(dat_kdv_control$ctx.lh.supramarginal, "WEI", density = TRUE, main = "(i)",ymax = ymax)
mBCCGo <- histDist(dat_kdv_control$ctx.lh.supramarginal, "BCCGo", density = TRUE, main = "(j)",ymax = ymax)
par(op)
...
## ctx.rh.supramarginal: mGA

op <- par(mfrow = c(4, 3))
mNO <- histDist(dat_kdv_control$ctx.rh.supramarginal, "NO", density = TRUE, main = "(a)",ymax=ymax)
mGA <- histDist(dat_kdv_control$ctx.rh.supramarginal, "GA", density = TRUE, main = "(b)",ymax=ymax)
mBCPE <- histDist(dat_kdv_control$ctx.rh.supramarginal, "BCPE", density = TRUE, main = "(c)",ymax = ymax)
mBCCG <- histDist(dat_kdv_control$ctx.rh.supramarginal, "BCCG", density = TRUE, main = "(d)",ymax = ymax)
mGG <- histDist(dat_kdv_control$ctx.rh.supramarginal, "GG", density = TRUE, main = "(e)",ymax = ymax)
mGIG <- histDist(dat_kdv_control$ctx.rh.supramarginal, "GIG", density = TRUE, main = "(f)",ymax = ymax)
mIG <- histDist(dat_kdv_control$ctx.rh.supramarginal, "IG", density = TRUE, main = "(g)",ymax = ymax)
mLOGNO <- histDist(dat_kdv_control$ctx.rh.supramarginal, "LOGNO", density = TRUE, main = "(h)",ymax = ymax)
mWEI <- histDist(dat_kdv_control$ctx.rh.supramarginal, "WEI", density = TRUE, main = "(i)",ymax = ymax)
mBCCGo <- histDist(dat_kdv_control$ctx.rh.supramarginal, "BCCGo", density = TRUE, main = "(j)",ymax = ymax)
par(op)
...
### 16th-84th percentile band (68.3% coverage)
pred_bcpe_left.caudate$q_lo_1sd <- qBCPE(0.1587, mu = pred_bcpe_left.caudate$mu, sigma = pred_bcpe_left.caudate$sigma,
                                          nu = pred_bcpe_left.caudate$nu, tau = pred_bcpe_left.caudate$tau)
pred_bcpe_left.caudate$q_hi_1sd <- qBCPE(0.8413, mu = pred_bcpe_left.caudate$mu, sigma = pred_bcpe_left.caudate$sigma,
                                          nu = pred_bcpe_left.caudate$nu, tau = pred_bcpe_left.caudate$tau)


### SD response
plot_mod_left.caudate_1sd<-ggplot(dat_kdv_control,aes(x=Age, y=left.caudate,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=left.caudate,colour=Sex)) +
  geom_line(data=pred_bcpe_left.caudate, aes(x=Age, y=mu,colour=Sex), size=.6) +
  geom_ribbon(data=pred_bcpe_left.caudate, aes(ymin=q_lo_1sd, ymax=q_hi_1sd, x=Age, y=mu,fill=Sex), alpha = 0.08) +# error band
...
plot_mod_left.caudate_95q<-ggplot(dat_kdv_control,aes(x=Age, y=left.caudate,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=left.caudate,colour=Sex)) +
  geom_line(data=pred_bcpe_left.caudate, aes(x=Age, y=mu,colour=Sex), size=.6) +
...
### 16th-84th percentile band (68.3% coverage)
pred_bcpe_right.caudate$q_lo_1sd <- qBCPE(0.1587, mu = pred_bcpe_right.caudate$mu, sigma = pred_bcpe_right.caudate$sigma,
                                           nu = pred_bcpe_right.caudate$nu, tau = pred_bcpe_right.caudate$tau)
pred_bcpe_right.caudate$q_hi_1sd <- qBCPE(0.8413, mu = pred_bcpe_right.caudate$mu, sigma = pred_bcpe_right.caudate$sigma,
                                           nu = pred_bcpe_right.caudate$nu, tau = pred_bcpe_right.caudate$tau)


### SD response
plot_mod_right.caudate_1sd<-ggplot(dat_kdv_control,aes(x=Age, y=right.caudate,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=right.caudate,colour=Sex)) +
  geom_line(data=pred_bcpe_right.caudate, aes(x=Age, y=mu,colour=Sex), size=.6) +
  geom_ribbon(data=pred_bcpe_right.caudate, aes(ymin=q_lo_1sd, ymax=q_hi_1sd, x=Age, y=mu,fill=Sex), alpha = 0.08) +# error band
  scale_y_continuous(breaks = seq(2500, 6100, by = 500),limits=c(2000, 6100))+
...
### 95% response: 95 % prediction / quantile band
plot_mod_right.caudate_95q<-ggplot(dat_kdv_control,aes(x=Age, y=right.caudate,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=right.caudate,colour=Sex)) +
  geom_line(data=pred_bcpe_right.caudate, aes(x=Age, y=mu,colour=Sex), size=.6) +
...
### 16th-84th percentile band (68.3% coverage)
pred_ga_left.hippocampus$q_lo_1sd <- qGA(0.1587, mu = pred_ga_left.hippocampus$mu, sigma = pred_ga_left.hippocampus$sigma)
pred_ga_left.hippocampus$q_hi_1sd <- qGA(0.8413, mu = pred_ga_left.hippocampus$mu, sigma = pred_ga_left.hippocampus$sigma)


### SD response
plot_mod_left.hippocampus_1sd<-ggplot(dat_kdv_control,aes(x=Age, y=left.hippocampus,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=left.hippocampus,colour=Sex)) +
  geom_line(data=pred_ga_left.hippocampus, aes(x=Age, y=mu,colour=Sex), size=.6) +
  geom_ribbon(data=pred_ga_left.hippocampus, aes(ymin=q_lo_1sd, ymax=q_hi_1sd, x=Age, y=mu,fill=Sex), alpha = 0.08) +# error band
  scale_y_continuous(breaks = seq(2500, 6100, by = 500),limits=c(2000, 6100))+
...
### 95% response: 95 % prediction / quantile band
plot_mod_left.hippocampus_95q<-ggplot(dat_kdv_control,aes(x=Age, y=left.hippocampus,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=left.hippocampus,colour=Sex)) +
  geom_line(data=pred_ga_left.hippocampus, aes(x=Age, y=mu,colour=Sex), size=.6) +
...
### 16th-84th percentile band (68.3% coverage)
pred_ga_right.hippocampus$q_lo_1sd <- qGA(0.1587, mu = pred_ga_right.hippocampus$mu, sigma = pred_ga_right.hippocampus$sigma)
pred_ga_right.hippocampus$q_hi_1sd <- qGA(0.8413, mu = pred_ga_right.hippocampus$mu, sigma = pred_ga_right.hippocampus$sigma)


### SD response
plot_mod_right.hippocampus_1sd<-ggplot(dat_kdv_control,aes(x=Age, y=right.hippocampus,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=right.hippocampus,colour=Sex)) +
  geom_line(data=pred_ga_right.hippocampus, aes(x=Age, y=mu,colour=Sex), size=.6) +
  geom_ribbon(data=pred_ga_right.hippocampus, aes(ymin=q_lo_1sd, ymax=q_hi_1sd, x=Age, y=mu,fill=Sex), alpha = 0.08) +# error band
  scale_y_continuous(breaks = seq(2500, 6100, by = 500),limits=c(2000, 6100))+
...
### 95% response: 95 % prediction / quantile band
plot_mod_right.hippocampus_95q<-ggplot(dat_kdv_control,aes(x=Age, y=right.hippocampus,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=right.hippocampus,colour=Sex)) +
  geom_line(data=pred_ga_right.hippocampus, aes(x=Age, y=mu,colour=Sex), size=.6) +
...
### 16th-84th percentile band (68.3% coverage)
pred_ga_ctx.lh.superiorparietal$q_lo_1sd <- qGA(0.1587, mu = pred_ga_ctx.lh.superiorparietal$mu, sigma = pred_ga_ctx.lh.superiorparietal$sigma)
pred_ga_ctx.lh.superiorparietal$q_hi_1sd <- qGA(0.8413, mu = pred_ga_ctx.lh.superiorparietal$mu, sigma = pred_ga_ctx.lh.superiorparietal$sigma)


### SD response
plot_mod_ctx.lh.superiorparietal_1sd<-ggplot(dat_kdv_control,aes(x=Age, y=ctx.lh.superiorparietal,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=ctx.lh.superiorparietal,colour=Sex)) +
  geom_line(data=pred_ga_ctx.lh.superiorparietal, aes(x=Age, y=mu,colour=Sex), size=.6) +
  geom_ribbon(data=pred_ga_ctx.lh.superiorparietal, aes(ymin=q_lo_1sd, ymax=q_hi_1sd, x=Age, y=mu,fill=Sex), alpha = 0.08) +# error band
...
### 95% response: 95 % prediction / quantile band
plot_mod_ctx.lh.superiorparietal_95q<-ggplot(dat_kdv_control,aes(x=Age, y=ctx.lh.superiorparietal,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=ctx.lh.superiorparietal,colour=Sex)) +
  geom_line(data=pred_ga_ctx.lh.superiorparietal, aes(x=Age, y=mu,colour=Sex), size=.6) +
...
### 16th-84th percentile band (68.3% coverage)
pred_bcpe_ctx.rh.superiorparietal$q_lo_1sd <- qBCPE(0.1587, mu = pred_bcpe_ctx.rh.superiorparietal$mu, sigma = pred_bcpe_ctx.rh.superiorparietal$sigma,
                                                     nu = pred_bcpe_ctx.rh.superiorparietal$nu, tau = pred_bcpe_ctx.rh.superiorparietal$tau)
pred_bcpe_ctx.rh.superiorparietal$q_hi_1sd <- qBCPE(0.8413, mu = pred_bcpe_ctx.rh.superiorparietal$mu, sigma = pred_bcpe_ctx.rh.superiorparietal$sigma,
                                                     nu = pred_bcpe_ctx.rh.superiorparietal$nu, tau = pred_bcpe_ctx.rh.superiorparietal$tau)


### SD response
plot_mod_ctx.rh.superiorparietal_1sd<-ggplot(dat_kdv_control,aes(x=Age, y=ctx.rh.superiorparietal,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=ctx.rh.superiorparietal,colour=Sex)) +
  geom_line(data=pred_bcpe_ctx.rh.superiorparietal, aes(x=Age, y=mu,colour=Sex), size=.6) +
  geom_ribbon(data=pred_bcpe_ctx.rh.superiorparietal, aes(ymin=q_lo_1sd, ymax=q_hi_1sd, x=Age, y=mu,fill=Sex), alpha = 0.08) +# error band
  scale_y_continuous(breaks = seq(9000, 17000, by = 500),limits=c(9000,17000))+
...
### 95% response: 95 % prediction / quantile band
plot_mod_ctx.rh.superiorparietal_95q<-ggplot(dat_kdv_control,aes(x=Age, y=ctx.rh.superiorparietal,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=ctx.rh.superiorparietal,colour=Sex)) +
  geom_line(data=pred_bcpe_ctx.rh.superiorparietal, aes(x=Age, y=mu,colour=Sex), size=.6) +
...
### 16th-84th percentile band (68.3% coverage)
pred_bcpe_ctx.lh.parsopercularis$q_lo_1sd <- qBCPE(0.1587, mu = pred_bcpe_ctx.lh.parsopercularis$mu, sigma = pred_bcpe_ctx.lh.parsopercularis$sigma,
                                                    nu = pred_bcpe_ctx.lh.parsopercularis$nu, tau = pred_bcpe_ctx.lh.parsopercularis$tau)
pred_bcpe_ctx.lh.parsopercularis$q_hi_1sd <- qBCPE(0.8413, mu = pred_bcpe_ctx.lh.parsopercularis$mu, sigma = pred_bcpe_ctx.lh.parsopercularis$sigma,
                                                    nu = pred_bcpe_ctx.lh.parsopercularis$nu, tau = pred_bcpe_ctx.lh.parsopercularis$tau)


### SD response
plot_mod_ctx.lh.parsopercularis_1sd<-ggplot(dat_kdv_control,aes(x=Age, y=ctx.lh.parsopercularis,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=ctx.lh.parsopercularis,colour=Sex)) +
  geom_line(data=pred_bcpe_ctx.lh.parsopercularis, aes(x=Age, y=mu,colour=Sex), size=.6) +
  geom_ribbon(data=pred_bcpe_ctx.lh.parsopercularis, aes(ymin=q_lo_1sd, ymax=q_hi_1sd, x=Age, y=mu,fill=Sex), alpha = 0.08) +# error band
...
### 95% response: 95 % prediction / quantile band
plot_mod_ctx.lh.parsopercularis_95q<-ggplot(dat_kdv_control,aes(x=Age, y=ctx.lh.parsopercularis,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=ctx.lh.parsopercularis,colour=Sex)) +
  geom_line(data=pred_bcpe_ctx.lh.parsopercularis, aes(x=Age, y=mu,colour=Sex), size=.6) +
...
### 16th-84th percentile band (68.3% coverage)
pred_no_ctx.rh.parsopercularis$q_lo_1sd <- qNO(0.1587, mu = pred_no_ctx.rh.parsopercularis$mu, sigma = pred_no_ctx.rh.parsopercularis$sigma)
pred_no_ctx.rh.parsopercularis$q_hi_1sd <- qNO(0.8413, mu = pred_no_ctx.rh.parsopercularis$mu, sigma = pred_no_ctx.rh.parsopercularis$sigma)


### SD response
plot_mod_ctx.rh.parsopercularis_1sd<-ggplot(dat_kdv_control,aes(x=Age, y=ctx.rh.parsopercularis,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=ctx.rh.parsopercularis,colour=Sex)) +
  geom_line(data=pred_no_ctx.rh.parsopercularis, aes(x=Age, y=mu,colour=Sex), size=.6) +
  geom_ribbon(data=pred_no_ctx.rh.parsopercularis, aes(ymin=q_lo_1sd, ymax=q_hi_1sd, x=Age, y=mu,fill=Sex), alpha = 0.08) +# error band
...
### 95% response: 95 % prediction / quantile band
plot_mod_ctx.rh.parsopercularis_95q<-ggplot(dat_kdv_control,aes(x=Age, y=ctx.rh.parsopercularis,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=ctx.rh.parsopercularis,colour=Sex)) +
  geom_line(data=pred_no_ctx.rh.parsopercularis, aes(x=Age, y=mu,colour=Sex), size=.6) +
...
### 16th-84th percentile band (68.3% coverage)
pred_ga_ctx.lh.superiortemporal$q_lo_1sd <- qGA(0.1587, mu = pred_ga_ctx.lh.superiortemporal$mu, sigma = pred_ga_ctx.lh.superiortemporal$sigma)
pred_ga_ctx.lh.superiortemporal$q_hi_1sd <- qGA(0.8413, mu = pred_ga_ctx.lh.superiortemporal$mu, sigma = pred_ga_ctx.lh.superiortemporal$sigma)


### SD response
plot_mod_ctx.lh.superiortemporal_1sd<-ggplot(dat_kdv_control,aes(x=Age, y=ctx.lh.superiortemporal,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=ctx.lh.superiortemporal,colour=Sex)) +
  geom_line(data=pred_ga_ctx.lh.superiortemporal, aes(x=Age, y=mu,colour=Sex), size=.6) +
  geom_ribbon(data=pred_ga_ctx.lh.superiortemporal, aes(ymin=q_lo_1sd, ymax=q_hi_1sd, x=Age, y=mu,fill=Sex), alpha = 0.08) +# error band
...
### 95% response: 95 % prediction / quantile band
plot_mod_ctx.lh.superiortemporal_95q<-ggplot(dat_kdv_control,aes(x=Age, y=ctx.lh.superiortemporal,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=ctx.lh.superiortemporal,colour=Sex)) +
  geom_line(data=pred_ga_ctx.lh.superiortemporal, aes(x=Age, y=mu,colour=Sex), size=.6) +
...
### 16th-84th percentile band (68.3% coverage)
pred_ga_ctx.rh.superiortemporal$q_lo_1sd <- qGA(0.1587, mu = pred_ga_ctx.rh.superiortemporal$mu, sigma = pred_ga_ctx.rh.superiortemporal$sigma)
pred_ga_ctx.rh.superiortemporal$q_hi_1sd <- qGA(0.8413, mu = pred_ga_ctx.rh.superiortemporal$mu, sigma = pred_ga_ctx.rh.superiortemporal$sigma)


### SD response
plot_mod_ctx.rh.superiortemporal_1sd<-ggplot(dat_kdv_control,aes(x=Age, y=ctx.rh.superiortemporal,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=ctx.rh.superiortemporal,colour=Sex)) +
  geom_line(data=pred_ga_ctx.rh.superiortemporal, aes(x=Age, y=mu,colour=Sex), size=.6) +
  geom_ribbon(data=pred_ga_ctx.rh.superiortemporal, aes(ymin=q_lo_1sd, ymax=q_hi_1sd, x=Age, y=mu,fill=Sex), alpha = 0.08) +# error band
...
### 95% response: 95 % prediction / quantile band
plot_mod_ctx.rh.superiortemporal_95q<-ggplot(dat_kdv_control,aes(x=Age, y=ctx.rh.superiortemporal,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=ctx.rh.superiortemporal,colour=Sex)) +
  geom_line(data=pred_ga_ctx.rh.superiortemporal, aes(x=Age, y=mu,colour=Sex), size=.6) +
...
### 16th-84th percentile band (68.3% coverage)
pred_ga_ctx.lh.supramarginal$q_lo_1sd <- qGA(0.1587, mu = pred_ga_ctx.lh.supramarginal$mu, sigma = pred_ga_ctx.lh.supramarginal$sigma)
pred_ga_ctx.lh.supramarginal$q_hi_1sd <- qGA(0.8413, mu = pred_ga_ctx.lh.supramarginal$mu, sigma = pred_ga_ctx.lh.supramarginal$sigma)


### SD response
plot_mod_ctx.lh.supramarginal_1sd<-ggplot(dat_kdv_control,aes(x=Age, y=ctx.lh.supramarginal,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=ctx.lh.supramarginal,colour=Sex)) +
  geom_line(data=pred_ga_ctx.lh.supramarginal, aes(x=Age, y=mu,colour=Sex), size=.6) +
  geom_ribbon(data=pred_ga_ctx.lh.supramarginal, aes(ymin=q_lo_1sd, ymax=q_hi_1sd, x=Age, y=mu,fill=Sex), alpha = 0.08) +# error band
  scale_y_continuous(breaks = seq(5000,13000, by = 500),limits=c(5000,13000))+
...
### 95% response: 95 % prediction / quantile band
plot_mod_ctx.lh.supramarginal_95q<-ggplot(dat_kdv_control,aes(x=Age, y=ctx.lh.supramarginal,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=ctx.lh.supramarginal,colour=Sex)) +
  geom_line(data=pred_ga_ctx.lh.supramarginal, aes(x=Age, y=mu,colour=Sex), size=.6) +
...
### 16th-84th percentile band (68.3% coverage)
pred_ga_ctx.rh.supramarginal$q_lo_1sd <- qGA(0.1587, mu = pred_ga_ctx.rh.supramarginal$mu, sigma = pred_ga_ctx.rh.supramarginal$sigma)
pred_ga_ctx.rh.supramarginal$q_hi_1sd <- qGA(0.8413, mu = pred_ga_ctx.rh.supramarginal$mu, sigma = pred_ga_ctx.rh.supramarginal$sigma)


### SD response
plot_mod_ctx.rh.supramarginal_1sd<-ggplot(dat_kdv_control,aes(x=Age, y=ctx.rh.supramarginal,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=ctx.rh.supramarginal,colour=Sex)) +
  geom_line(data=pred_ga_ctx.rh.supramarginal, aes(x=Age, y=mu,colour=Sex), size=.6) +
  geom_ribbon(data=pred_ga_ctx.rh.supramarginal, aes(ymin=q_lo_1sd, ymax=q_hi_1sd, x=Age, y=mu,fill=Sex), alpha = 0.08) +# error band
...
### 95% response: 95 % prediction / quantile band
plot_mod_ctx.rh.supramarginal_95q<-ggplot(dat_kdv_control,aes(x=Age, y=ctx.rh.supramarginal,colour=Sex,fill=Sex)) +
  #geom_point(aes(colour=Sex)) +
  geom_point(data=dat_kdv_patient,aes(x=Age,y=ctx.rh.supramarginal,colour=Sex)) +
  geom_line(data=pred_ga_ctx.rh.supramarginal, aes(x=Age, y=mu,colour=Sex), size=.6) +
